...
async def crole_update(channel_id:str,_type:str,_value:str,_allow:int,*_deny:int):
    ...
    async with aiohttp.ClientSession() as session:
        async with session.post(url3, data=params3,headers=kook_headers) as response:
                ...
                _log.debug(ret3)
    return ret3
async def crole_list(guild_id:str):
    urlc = kook_base + f"/api/v3/guild-role/list?guild_id={guild_id}"
    async with aiohttp.ClientSession() as session:
        async with session.post(urlc, headers=kook_headers) as response:
                retd =json.loads(await response.text())
                _log.debug(retd)
    return retd
