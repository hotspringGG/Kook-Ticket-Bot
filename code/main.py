...
async def gaming(msg: Message, game: int = 0, *arg):
    ...
    try:
        if game == 0:
            await msg.reply(f"[gaming] 参数错误，用法「/gaming 数字」\n1-Minecraft，2-英雄联盟，3-CSGO")
            return
        elif game == 1:
            ret = await status_active_game(1356599)  # 人间地狱
            await msg.reply(f"{ret['message']}，Bot上号Minecraft啦！")
        elif game == 2:
            ...
            send_msg = await msg.ctx.channel.send(
                CardMessage(
                    Card(
                        Module.Section(
                            """杉木镇工单帮助
如果你在游玩本服务器遇到了问题，发现了Bug，或者你对服务器有改进的建议，都可以点击右边的按钮发起工单，服务器的管理人员会和你进行沟通。
			                """,
                            Element.Button(
                                "创建工单", value=values, click=Types.Click.RETURN_VAL
                            ),
                        )
                    )
                )
    ...
    async with TicketOpenLock:
        ...
        try:
            ...
            if e.body["target_id"] in TKconf["ticket"]["channel_id"]:
                ...
                #禁用全体成员的
                await crole_create(ret1["data"]["id"], "role_id", "0")
                await crole_update(ret1["data"]["id"], "role_id", "0", 0, 2048)
                await asyncio.sleep(0.2)  # 休息一会 避免超速

                #await crole_create(ret1["data"]["id"], "role_id",)
                # 3.先设置管理员角色的权限
                # 全局管理员
                for rol in TKconf["ticket"]["admin_role"]:
                    ...
                    # 设置该频道的角色权限为可见
                    await crole_update(ret1["data"]["id"], "role_id", rol, 2048, 0)
                    await asyncio.sleep(0.2)  # 休息一会 避免超速
                # 4.频道管理员
